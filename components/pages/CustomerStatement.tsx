...
export default function CustomerStatement({ customerId }: CustomerStatementProps) {
  ...
  if (error || !data) {
    return (
      <Card className="border-destructive dir-rtl" dir="rtl">
        <CardHeader>
          <CardTitle className="text-destructive font-bold">خطأ في تحميل كشف الحساب</CardTitle>
          <CardDescription>تعذر استرداد سجل العميل في الوقت الحالي. يرجى المحاولة مرة أخرى لاحقاً.</CardDescription>
        </CardHeader>
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-8 dir-rtl font-arabic pb-10" dir="rtl">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 print:hidden">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-primary mb-2">كشف حساب عميل</h1>
          <p className="text-muted-foreground text-lg">سجل مفصل لجميع المعاملات المالية والفواتير الخاصة بالعميل.</p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" size="lg" onClick={handlePrint} className="rounded-xl border-2 hover:bg-secondary">
            <Printer className="ml-2 h-5 w-5" />
            طباعة الكشف
          </Button>
          <Button variant="default" size="lg" className="rounded-xl shadow-lg shadow-primary/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-r-4 border-r-primary overflow-hidden transition-all hover:shadow-md">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-bold text-muted-foreground">بيانات العميل</CardTitle>
        ...
        <Card className={`border-r-4 ${currentBalance > 0 ? 'border-r-destructive' : 'border-r-emerald-500'} overflow-hidden transition-all hover:shadow-md`}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-bold text-muted-foreground">الرصيد الحالي</CardTitle>
            <div className={`p-2 ${currentBalance > 0 ? 'bg-destructive/10' : 'bg-emerald-500/10'} rounded-full`}>
              <Wallet className={`h-4 w-4 ${currentBalance > 0 ? 'text-destructive' : 'text-emerald-500'}`} />
            </div>
          </CardHeader>
          <CardContent>
            <div className={`text-3xl font-black ${currentBalance > 0 ? 'text-destructive' : 'text-emerald-600'}`}>
              {Math.abs(currentBalance).toLocaleString()} <span className="text-sm font-bold text-muted-foreground">د.ع</span>
            </div>
        ...
        <Card className="border-r-4 border-r-blue-500 overflow-hidden transition-all hover:shadow-md">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-bold text-muted-foreground">إجمالي العمليات</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs font-bold text-muted-foreground mt-2">إجمالي الحركات في السجل</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardHeader className="bg-muted/30 pb-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <CardDescription className="font-medium">عرض تفصيلي لجميع الفواتير والمدفوعات مرتبة زمنياً.</CardDescription>
            </div>
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-20">
                      <div className="flex flex-col items-center gap-3 text-muted-foreground">
                        ...
                        <p className="text-lg font-bold">لا توجد عمليات مطابقة للبحث</p>
                      </div>
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredStatement.map((entry: any, idx: number) => (
                    <TableRow key={entry.id} className={`${idx % 2 === 0 ? 'bg-transparent' : 'bg-muted/10'} hover:bg-muted/20 transition-colors`}>
                      <TableCell className="font-bold text-muted-foreground whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <Calendar className="h-3 w-3 text-primary/40" />
                          {format(new Date(entry.date), "yyyy/MM/dd", { locale: ar })}
                        </div>
                      </TableCell>
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          <span className="font-black text-foreground">{entry.title}</span>
                          {entry.description && (
                            <span className="text-sm text-muted-foreground font-medium">{entry.description}</span>
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        ...
                        >
                          {entry.type === 'INVOICE' ? (
                            <><ArrowUpRight className="ml-1 h-3.5 w-3.5" /> فاتورة</>
                          ) : (
                            <><ArrowDownLeft className="ml-1 h-3.5 w-3.5" /> دفعة مالية</>
                          )}
                        </Badge>
                      </TableCell>
      ...
      <div className="hidden print:flex flex-col items-center mt-12 gap-4 border-t-2 pt-8">
        <div className="text-center">
          <p className="text-xl font-black text-primary">نظام المحاسبة المتكامل</p>
          <p className="text-sm font-bold text-muted-foreground mt-1">تم استخراج هذا الكشف بتاريخ: {format(new Date(), "PPPP", { locale: ar })}</p>
        </div>
